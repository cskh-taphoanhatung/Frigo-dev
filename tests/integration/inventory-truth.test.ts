...
const migration = readFileSync('migrations/0024_inventory_truth_foundation.sql', 'utf8');
...
describe('T08 inventory truth persistence and legacy backfill', () => {
  ...
  it('replays the full migration ledger on a fresh database with no automatic lot cutover', () => {
    const db = database();
    // Ledger length/tip are asserted by the migration-specific suites; Inventory Truth only needs
    // every inventory migration applied and no implicit lot cutover.
    expect(db.migrations).toContain('0033_scan_evidence_completeness.sql');
  ...
  it('upgrades a populated 0022 database without changing legacy rows or event history', async () => {
    ...
    db.seed(readFileSync('migrations/0023_scan_request_fingerprint.sql', 'utf8'));
    db.seed(migration);
    for (const file of readdirSync('migrations').filter((file) => /^\d+.*\.sql$/.test(file) && file >= '0025').sort()) {
      db.seed(readFileSync(`migrations/${file}`, 'utf8'));
    }
    ...
    // 0025 adds the nullable command linkage without touching event history.
