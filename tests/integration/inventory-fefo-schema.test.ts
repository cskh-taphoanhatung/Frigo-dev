...
describe('T09E SQLite FEFO authority guards', () => {
  ...
  it('retains v1 rows byte-for-byte across the populated 0027 upgrade, replays v1, and accepts new FEFO', async () => {
    ...
    databases.push(db);
    for (const file of readdirSync('migrations').filter((name) => /^\d+.*\.sql$/.test(name) && name <= '0027_inventory_event_poststate.sql').sort()) {
    ...
    const before = bytes();
    db.seed(readFileSync('migrations/0028_inventory_fefo_authority.sql', 'utf8'));
    ...
    db.seed(readFileSync('migrations/0030_inventory_fefo_backfill_compatibility.sql', 'utf8'));
