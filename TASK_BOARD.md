...
- [done, PR #27 / T15A-R2, on branch] Workflow wiring (`catalog` step in `production-d1-migrate.yml` after `verify`/before the schema gate, input interpolation removed, staging+production deploy proof via the helper) and unconditional guardrail tests are committed on the PR branch (`366dbd1`, `776422f`; identical to `t15a-r/workflows.patch` / `r2-wired-series.mbox`, retained as audit evidence). Focused 123/0 skipped; hosted exact-head CI SUCCESS. Classification `T15A_R2_WIRED_PRE_MERGE`.
- [next, needs merge authorization] Merge PR #27 (merge commit, expected-head guard) → exact-main CI → observe automatic staging deploy (release+staging SUCCESS, exact-SHA convergence, production SKIPPED). T15A pre-production hardening is NOT complete until then. Production rollout still NOT started.
