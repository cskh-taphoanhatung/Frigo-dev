...
## Canonical lineage refresh — 2026-09-15 UTC

`T14A_AUDIT_BASE_MAIN=345cecf388321a00c96be387744a10e8c98bd9ac` is the
**historical audit baseline**, not current main.
`CURRENT_CANONICAL_MAIN=055913596acd3bef049aa0eb007bde2873a658a9` is the
freshly fetched integration baseline in `vn-clo/Frigo-dev` (ID `1368281478`).
Later integration/merge receipts belong in `T14_INTEGRATION_REFRESH.md`.

PR #9 merged as `911db7fdddcd60ea1e3f3c17b4aed3f4b922bda5`; PR #10 merged
as `055913596acd3bef049aa0eb007bde2873a658a9`. Their combined application
delta from the audit baseline is limited to Auth/GIS, OCR pending UX and COOP
regression coverage. Git comparisons verified no changes to recipe packages,
DB readers, recipe/Week/planner routes, recipe media or migrations. The recipe
audit remains applicable; its production deployment and CI receipts are historical.

PR #10 changes only `PROJECT_STATUS.md`, `TASK_BOARD.md`, and
`docs/ai/{CURRENT_STATE,HANDOFF,TASK_BOARD}.md` after PR #9. The application-path
diff from PR #9 merge to the integration baseline is empty. The recorded deployed
application lineage is now PR #9, with Worker version
`20bc1f35-6ffe-4085-ba79-d54a0b53da71` at 100% according to its rollout receipt;
the earlier `e6b9195` release is superseded. This refresh did not deploy or access
production resources.

Current disposition of historical PR #4 is **CLOSE_ARCHIVE**, not merge; its
branch/history remains untouched. T14B-A is technically accepted in source PR #8
but not yet integrated at this checkpoint. T14B-B is **not started**; migration
0034, recipe-data changes and runtime-authority changes are not authorized here.
Recipe media remains deferred to T14C.

## Original audit — historical checkpoint at `345cecf`

All “current”, “deployed”, “VERIFIED” and next-action statements in the original
audit below are scoped to that historical checkpoint, not the refreshed main.
Its findings and evidence are retained without rewriting the original investigation.
...
| Section | Content |
| --- | --- |
| §1 | VERIFIED HISTORICAL AUDIT STATE — repository, production and main identity |
...
## 1. VERIFIED HISTORICAL AUDIT STATE
