...
export const PlannerPage: React.FC = () => {
  ...
  return <PlannerShell {...language} plan={setup ? null : plan}>
    ...
    {model.refreshed && <p role="status" className="text-sm text-emerald-900 bg-emerald-50 p-3 rounded-xl">{t.updated}</p>}
    {setup ? <PlannerSetup model={model} locale={locale} /> : model.query.isPending ? <div role="status" className="py-16 text-center text-emerald-900"><CalendarDays className="mx-auto mb-4 animate-pulse" size={36} /><p>{t.loading}</p></div>
      ...
      : !plan ? <Card className="text-center !py-10"><CalendarDays className="mx-auto mb-5 text-emerald-700" size={40} /><h2 className="text-xl font-heading font-bold">{t.empty}</h2><p className="mt-3 text-sm text-slate-600 leading-relaxed">{t.emptyText}</p><Link to="/planner/new" className="inline-flex min-h-12 mt-6 items-center justify-center rounded-xl bg-takosan-green px-5 py-3 text-sm font-semibold text-white">{t.newPlan}</Link></Card>
